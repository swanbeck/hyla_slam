...
DisableLocalization::DisableLocalization(const std::string name, const BT::NodeConfig &config)
: BT::StatefulActionNode(name, config), node_(rclcpp::Node::make_shared(name))
{}

BT::PortsList DisableLocalization::providedPorts()
{
    return {
...
BT::NodeStatus DisableLocalization::onStart()
{
    ...
    std::string service_handle {"hyla_slam/disable_localization"};
    ...
    while (!service_client_->wait_for_service(std::chrono::milliseconds(10))) {
        if (std::chrono::steady_clock::now() - start_time > timeout) {
            RCLCPP_ERROR(node_->get_logger(), "Timed out waiting for disable localization service!");
    ...
    RCLCPP_INFO_STREAM(node_->get_logger(), "Disable localization " << result << "...");

    return result;
}

BT::NodeStatus DisableLocalization::onRunning()
{
    ...
    switch(response) {
        case rclcpp::FutureReturnCode::TIMEOUT: {
            RCLCPP_DEBUG(node_->get_logger(), "Timed waiting for disable localization response! Still running...");
            return BT::NodeStatus::RUNNING;
        }
        case rclcpp::FutureReturnCode::INTERRUPTED: {
            RCLCPP_ERROR(node_->get_logger(), "Interrupted waiting for disable localization response. Aborting...");
            return BT::NodeStatus::FAILURE;
        }
        case rclcpp::FutureReturnCode::SUCCESS: {
            ...
            RCLCPP_INFO_STREAM(node_->get_logger(), "Disable localization responded! Returning " << return_status << "!");
...
void DisableLocalization::onHalted()
{
