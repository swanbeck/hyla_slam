#include "hyla_slam_genz_behaviors_node.hpp"
...
: rclcpp::Node("hyla_slam", opts), localization_enabled_(false)
{
    ...
    // handle parameters
    param_listener_ = std::make_shared<hyla_slam_genz::ParamListener>(get_node_parameters_interface());
    ...
    localization_config_.map_cleanup_radius = localization_params.map_cleanup_radius;
    localization_config_.planarity_threshold = localization_params.planarity_threshold;
    localization_config_.max_points_per_voxelized_scan = localization_params.max_points_per_voxelized_scan;
    localization_config_.min_points_per_voxelized_scan = localization_params.min_points_per_voxelized_scan;
    localization_config_.deskew = localization_params.deskew;
    localizer_ = std::make_unique<hyla_genz::HylaGenz>(localization_config_);
...
void BehaviorsNode::setLocalizationEstimate(const std::shared_ptr<hyla_slam_interfaces::srv::SetPose::Request> request, std::shared_ptr<hyla_slam_interfaces::srv::SetPose::Response>)
{
    ...
    RCLCPP_INFO_STREAM(this->get_logger(), "Computed pose...");
    ...
    RCLCPP_INFO_STREAM(this->get_logger(), "Set in localizer...");
